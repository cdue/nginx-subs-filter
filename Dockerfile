...
RUN curl https://nginx.org/download/$nginx_version.tar.gz -o nginx.tar.gz && \
    tar -xzvf nginx.tar.gz
RUN cd nginx-$nginx_version && ./configure --with-compat --add-dynamic-module=../ngx_http_substitutions_filter_module && make modules

FROM nginx:latest
#FROM nginx:stable
COPY --from=0 /tmp/nginx-$nginx_version/objs/ngx_http_subs_filter_module.so /etc/nginx/modules/
